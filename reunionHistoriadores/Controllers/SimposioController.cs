...
{
    public class SimposioController : Controller
    {
        private readonly IErrorLog _errorlog;
        private readonly RegistroContext _context;
        private readonly GlobalOptions _opciones;
        private readonly IStringLocalizer<SimposioController> _localizer;
        ...
        public SimposioController(IErrorLog errorlog, RegistroContext context, IOptionsSnapshot<GlobalOptions> opciones, IStringLocalizer<SimposioController> localizer, IStringLocalizer<SharedResource> sharedLocalizer, IEmail email)
        {
            _errorlog = errorlog;
            _context = context;
        ...
        public async Task<IActionResult> Simposio()
        {
            if (!_registroEnabled)
            ...
            var model = new SimposioViewModel
            {
                RegistroTipo = RegistroTipo,
                Mesas = new List<RegistroMesaViewModel>
                {
                    new RegistroMesaViewModel()
                    {
                        Ponencias = new List<PonenciaSimposioViewModel>
                        {
                            new PonenciaSimposioViewModel(),
                            new PonenciaSimposioViewModel(),
                            new PonenciaSimposioViewModel(),
                            new PonenciaSimposioViewModel()
                        }
        ...
        public async Task<IActionResult> Simposio(int id, SimposioViewModel model)
        {
            if (!ExisteTipoRegistroVigente(id))
                return NotFound();
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    var registro = new Registro
                    {
                        ...
                        ExposicionTipo = 3, //3-Tipo Simposio 
                    };
                    _context.Registros.Add(registro);
                    _context.SaveChanges();

                    var simposio = new Simposio
                    {
                        Nombre = model.NombreSimposio,
                        LineaTematicaId = model.LineaTematicaId,
                        LineaTematicaOtro = model.LineaTematicaOtro,
                        CoordinadorNombre = model.NombreCoordinador,
                        CoordinadorGradoAcademico = model.GradoAcademicoCoordinador,
                        CoordinadorInstitucion = model.InstitucionCoordinador,
                        CoordinadorCorreo = model.CorreoCoordinador,
                        RegistroId = registro.Id,
                        Visible = true,
                        Archivos = new List<Archivo>()
                    };
                    _context.Add(simposio);
                    _context.SaveChanges();
                    ...
                    using (var memoryStream = new MemoryStream())
                    {
                        ...
                        simposio.Archivos.Add(file);
                    }

                    //Mesas
                    int numeroMesa = 1;
                    foreach (var mesaVM in model.Mesas)
                    {
                        if (mesaVM.NombreCoordinador != null)
                        {
                            var mesa = new Mesa
                            {
                                Nombre = "Mesa " + numeroMesa + "-" + simposio.Nombre,
                                CoordinadorNombre = mesaVM.NombreCoordinador,
                                CoordinadorCorreo = mesaVM.CorreoCoordinador,
                                RegistroId = registro.Id,
                            ...
                            using (var memoryStream = new MemoryStream())
                            {
                                await mesaVM.Resumen.CopyToAsync(memoryStream);

                                var file = new Archivo
                                {
                                    Contenido = memoryStream.ToArray(),
                                    ArchivoTipoId = TipoIdentificacion.Id,
                                    Mime = mesaVM.Resumen.ContentType,
                                    Nombre = mesaVM.Resumen.FileName,
                                    Fecha = DateTime.Now,
                                    Size = mesaVM.Resumen.Length
                                };
                                mesa.Archivos.Add(file);
                            }

                            //Ponencias
                            foreach (var ponenciaVM in mesaVM.Ponencias)
                            {
                    ...
                    // Envía correo
                    //await _email.EnviaCorreoAsync($"[{ViewData["SiteShortTitle"]}] {_localizer["Registro"]} {ViewData["SiteTitle"]}", model.Correo, null, null, CuerpoCorreo(itemToCreate.Id));
